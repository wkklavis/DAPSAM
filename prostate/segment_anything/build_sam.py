...
def _build_sam(
    ...
    if checkpoint is not None:
        ...
        try:
            ...
            new_state_dict = load_from_h(sam, state_dict, image_size, vit_patch_size, encoder_global_attn_indexes)
...
def load_from_h(sam, state_dict, image_size, vit_patch_size, encoder_global_attn_indexes):
    sam_dict = sam.state_dict()
    except_strings = []
    except_keys = ['mask_tokens', 'output_hypernetworks_mlps', 'iou_prediction_head']
    new_state_dict = {}
    for k, v in state_dict.items():
        if k in sam_dict.keys() and not check_contain(k, except_strings) and except_keys[0] not in k and except_keys[1] not in k and except_keys[2] not in k:
            new_state_dict[k] = v
    pos_embed = new_state_dict["image_encoder.pos_embed"]
    token_size = int(image_size // vit_patch_size)
    if pos_embed.shape[1] != token_size:
        ...
        for k in rel_pos_keys:
            flag = any([(str(j) == k.split(".")[2]) for j in encoder_global_attn_indexes])
            if flag:
